...
Import-Module (Join-Path $PSScriptRoot 'EiImagePreprocess.psm1') -Force
...
function Write-EiPayload {
    ...
    $chunkSize = 128
    $pauseTicks = [long]([System.Diagnostics.Stopwatch]::Frequency / 1000)
...
$serialLogPath = Join-Path $PSScriptRoot ($RunName + '_serial.log')
...
try {
    ...
    while ([DateTime]::UtcNow -lt $readyDeadline) {
        ...
        if ($line.StartsWith('EI_BATCH_READY|')) {
            ...
            if ($readyFields['width'] -ne '96' -or
                $readyFields['height'] -ne '96' -or
                $readyFields['channels'] -ne '3' -or
                $readyFields['bytes'] -ne '27648') {
    ...
    for ($inventoryIndex = 0; $inventoryIndex -lt $inventory.Count; ++$inventoryIndex) {
        ...
        $prepared = ConvertTo-EiRgb888 -SourcePath $sourcePath
        if ($prepared.Rgb.Length -ne 27648) {
        ...
        $crc32 = Get-EiCrc32 -Data $prepared.Rgb
        ...
        $serialLog.WriteLine(
            "PC_IMAGE|id=$imageId|file=$($record.File)|expected=$($record.Expected)|sha256=$actualHash|crc32=$('{0:X8}' -f $crc32)")
        ...
        $responseDeadline = [DateTime]::UtcNow.AddSeconds(20)
