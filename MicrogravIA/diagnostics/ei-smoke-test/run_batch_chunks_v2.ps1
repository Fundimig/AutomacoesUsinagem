...
$runnerPath = Join-Path $PSScriptRoot 'run_batch_validation_v2.ps1'
...
$runsPath = Join-Path $resolvedOutputDirectory ($RunName + '_runs.csv')
$serialPath = Join-Path $resolvedOutputDirectory ($RunName + '_serial.log')
...
for ($chunkStartIndex = $StartIndex; $chunkStartIndex -lt $inventory.Count; $chunkStartIndex += $ChunkSize) {
    ...
    for ($attempt = 1; $attempt -le $MaxAttempts -and -not $completed; ++$attempt) {
        ...
        $chunkResultsPath = Join-Path $resolvedOutputDirectory ($chunkName + '_results.csv')
        $chunkBoxesPath = Join-Path $resolvedOutputDirectory ($chunkName + '_boxes.csv')
        $chunkSerialPath = Join-Path $resolvedOutputDirectory ($chunkName + '_serial.log')
